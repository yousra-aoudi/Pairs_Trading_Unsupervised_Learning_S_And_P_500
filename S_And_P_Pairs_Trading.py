...
Our goal in this case study is to perform clustering analysis on the stocks of S&P500 and come up with pairs for a pairs
trading strategy.

The data of the stocks of S&P 500, obtained using pandas_datareader from yahoo finance. It includes price data from
2020 onwards.
...
dataset = read_csv('SP500Data.csv',index_col=0, parse_dates=True)
...
plt.savefig('optimal number of clusters using the Elbow method.png')
plt.show()
...
plt.grid(True)
plt.title('Optimal number of clusters using the Silhouette method')
plt.savefig('optimal number of clusters using the Silhouette method.png')
plt.show()
...
plt.savefig('Cluster Member Counts.png')
plt.show()

"""
The number of stocks in a cluster range from around 20 to 160. Although, the distribution is not equal, we have 
a good number of stocks in each cluster.
"""
...
plt.savefig("Stocks Dendrograms.png")
plt.show()
...
Choosing a threshold cut at 14 yields four clusters, as confirmed in the following Python code:
"""
...
plt.xlabel('Stocks in Cluster')
plt.ylabel('Cluster Number')
plt.savefig('Stocks in cluster.png')
plt.show()
...
km 0.3395536431179637
hc 0.34088557776483275
ap 0.3449466149876631
-----------
Given the Affinite propagation performs the best, we will use it for the next step of our case study, with a number of 
cluster = 32.
"""
...
for clust in cluster_vis_list[0:min(len(cluster_vis_list), 4)]:
    ...
    means = np.log(dataset.loc[:"2022-05-01", tickers].mean())
    data = np.log(dataset.loc[:"2022-05-01", tickers]).sub(means)
